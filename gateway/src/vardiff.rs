...
pub struct Vardiff {
    ...
    pub last_sent: u64,
    /// A floor above `min` set for a fingerprinted miner (NiceHash).
    pub forced_floor: u64,
    /// Whether the job in force is a quick-raise (`Q`) job, and the difficulty it carries.
    pub quickdiff_active: bool,
    pub quickdiff_value: u64,
    snap_count: u64,
    snap_at: Instant,
}
...
impl Vardiff {
    ...
    pub fn update(&mut self, no_quick: bool, now: Instant) -> bool {
            ...
            && ms_per_share < target_ms / p.quickdiff_delta.max(1)
        {
            ...
            let rounded = if raw == 0 { 1 } else { 1u64 << (63 - raw.leading_zeros()) };
            self.current = rounded.max(self.current << 2);
            self.reset_snapshot(now);
            return true;
        }
...
mod tests {
    ...
    fn eight_shares_in_two_seconds_quick_raise_by_the_measured_factor() {
        ...
        assert_ne!(v.current, v.last_sent, "the caller must announce it");
    }
